...
class PLTDZVEngLogController extends Controller
{
    public function view(Request $r){
        ...
        return view('pltd-amp.zv-eng-log', compact('unit'));
    }
	...
	public function loadData(Request $r){
		if($r->date == null){
			$log = PLTDZVEngLog::with('users')->with('pltdUnit')->where('pltd_unit_id', $r->unit_id)->orderBy('id','desc')->get();
		}else{
			$log = PLTDZVEngLog::with('users')->with('pltdUnit')->where('tanggal', $r->date)->where('pltd_unit_id', $r->unit_id)->orderBy('id','desc')->get();
		}
		return compact('log');
	}
